...
class StatusReactiveRepositoryAdapterTest {
  ...
  void findByIdShouldReturnStatus() {
    ...
    when(mapper.mapBuilder(statusEntity, Status.Builder.class)).thenReturn(
      Status.builder().statusId(statusEntity.getStatus_id()).name(statusEntity.getName()).description(statusEntity.getDescription()));
    Mono<Status> result = adapter.findById(statusId);
    StepVerifier
      .create(result)
      .assertNext(st -> assertAll(() -> assertEquals(status.getStatusId(), st.getStatusId()), () -> assertEquals(status.getName(), st.getName()),
